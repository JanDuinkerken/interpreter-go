...
func TestIdentifierExpression(t *testing.T) {
	...
	if len(program.Statements) != 1 {
		t.Fatalf("The number of statements is different than 1. got=%d", len(program.Statements))
	}
...
func TestParsingPrefixExpressions(t *testing.T) {
	...
	for _, tt := range prefixTests {
		...
		if len(program.Statements) != 1 {
			t.Fatalf("program.Statements length different than 1. got=%d", len(program.Statements))
		}
		stmt, ok := program.Statements[0].(*ast.ExpressionStatement)
		if !ok {
...
func TestParsingInfixExpressions(t *testing.T) {
	...
	for _, tt := range infixTests {
		...
		if !testInfixExpression(t, stmt.Expression, tt.leftValue, tt.operator, tt.rightValue) {
			t.Fatalf("stmt.Expression is not a correct expression")
		}
	}
}
...
func TestIfExpression(t *testing.T) {
	...
	if !testInfixExpression(t, exp.Condition, "x", "<", "y") {
		t.Fatalf("exp.Condition is not a correct expression")
	}
...
func TestIfElseExpression(t *testing.T) {
	...
	if len(exp.Alternative.Statements) != 1 {
		t.Fatalf("consequence is not 1 statement. got=%d", len(exp.Alternative.Statements))
	}
...
func TestFunctionLiteralParsing(t *testing.T) {
	...
	if len(function.Parameters) != 2 {
		t.Fatalf("function literal parameters are not 2. got=%d", len(function.Parameters))
	}
	...
	if len(function.Body.Statements) != 1 {
		t.Fatalf("function.Body.Statements length is not 1. got=%d", len(function.Body.Statements))
	}
...
func TestCallExpressionParsing(t *testing.T) {
	...
	if len(callExp.Arguments) != 3 {
		t.Fatalf("callExp.Arguments length is not 3. got=%d", len(callExp.Arguments))
	}
